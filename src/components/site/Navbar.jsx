    ...
    >
      <div
        className={`mx-auto flex max-w-7xl items-center justify-between gap-2 px-5 transition-all duration-300 md:px-7 lg:gap-3 xl:gap-4 ${
          scrolled
            ? "mt-2 rounded-2xl glass py-2.5 shadow-lift"
            : "mt-3 rounded-2xl bg-white/70 py-3.5 backdrop-blur-md"
        } w-[calc(100%-1.5rem)]`}
      >
        ...
        <nav className="hidden items-center lg:flex" onMouseLeave={() => setHoverItem(null)}>
          {NAV.map((item, i) => (
            <motion.div
              key={item.label}
              className="relative"
                ...
                {NAV.map((item, i) => (
                  ...
                  >
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-base font-medium text-foreground/80 transition-colors hover:bg-secondary hover:text-primary"
                    >
                      {item.label}
                      {item.children && (
                        <motion.span
                          animate={{ rotate: mobileOpen === item.label ? 180 : 0 }}
                          transition={{ duration: 0.2 }}
                        >
                          ...
                          <div className="ml-2 space-y-0.5 border-l border-border py-1 pl-3">
                            {item.children.map((c) => (
                              <button
                                key={c.label}
                                onClick={() => go(c.to)}
                                className="block w-full rounded-lg px-3 py-2 text-left text-sm text-foreground/70 transition-colors hover:bg-secondary hover:text-primary"
                              >
                                {c.label}
                              </button>
                            ))}
